...
#--------FUNCTIONS--------------------------------------------
def again(): #header of function
    ans = input("\t\tWould you like to enter another temperature? [y/n]: ").lower()

    #error trap loop to ensure user provides valid value
    while ans != "y" and ans != "n":
        print("***This Value is Invalid, Resubmit Entry!***")
        ans = input("\t\tWould you like to enter another temperature? [y/n]: ").lower()
    
    return ans #value will replace the function call in the main code
    print("Wake Up Samurai We Got A City To Burn - Johnny Silverhand")
...
while again():
    ...
    #print(f"\n\t\tTEMP# {temp_count}\tTEMP {tmpF:.1f}F = TEMP {tempC:.1f}C\n") does the same thing
